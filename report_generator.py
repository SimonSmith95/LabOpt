...
def generate_report(
    ...
    """
    session_name = metadata.get("session_name", "BHOP Session")
...
<title>BHOP Report — {session_name}</title>
<style>
{_CSS}
</style>
</head>
...
<h1>🔬 BHOP Optimisation Report</h1>
...
<footer>
  Generated by BHOP &mdash; {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
...
def write_report(html: str, directory: str) -> str:
    ...
    path = os.path.join(directory, f"bhop_report_{ts}.html")
...
def export_plots_as_png(
    ...
    ``/sessions/bhop_report_20260908_221901.html``, the folder will be
    ``/sessions/bhop_report_20260908_221901_plots/`` and will contain
